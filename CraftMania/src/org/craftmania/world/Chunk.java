...
public class Chunk implements AABBObject
{
	...
	{
		Chunk chunk = getBlockChunkContaining(x, y, z, false, false, false);
	...
	{
		Chunk chunk = getBlockChunkContaining(x, y, z, createIfNecessary, loadIfNecessary, generateIfNecessary);
		if (chunk != null)
		{
	...
	public Chunk getNeighborBlockChunk(Side side)
	{
	...
	public Chunk getBlockChunkContaining(int x, int y, int z, boolean createIfNecessary, boolean loadIfNecessary, boolean generateIfNecessary)
	{
		...
		{
			Chunk neighbor = getNeighborBlockChunk(side);
			if (neighbor == null)
			{
				// return null;
	...
	public synchronized void destroy()
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			Chunk neighbor = this.getNeighborBlockChunk(side);
			if (neighbor != null)
			{
